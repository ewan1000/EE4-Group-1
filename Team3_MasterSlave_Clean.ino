...
void setup() {
  ...
  gpio_pull_up(5);
  if (MasterInit == 1){     //Master Setup: Team3 is configured to Master on I2C0 channel
    Wire.end();  
    Wire.begin(); 
    Serial.begin(9600);

  }   
  else if(MasterInit == 0){            //Slave Setup: Team3 is configured to Slave on I2C0 channel
    Wire.end();
    Wire.begin(TEAM3);
    Serial.begin(9600); 
...
void receiveEvent(int x){   //in wire.receiveEvent definition, an int must be declared in scope
  ...
  if (Slave3Receives == 3 ){          //if team3 as slave receives a 3 from other team acting as master
    Serial.println(3);
