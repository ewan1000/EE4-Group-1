...
void receiveEvent(int howMany) // howMANY is always equal to no. of bytes received
  {                             //an int is 4 bytes, therefore taking up 4 spaces in the array                                    
    ...
    if (z1 >= 0)
      {
        while(stepper.currentPosition() != z1)    // Run the motor forward at 1500 steps/second until the motor reaches coord from array (1/16 revolutions)
          {
      ...
      {
       while(stepper.currentPosition() != z1)    // Run the motor backward at 1500 steps/second until the motor reaches coord from array (1/16 revolutions)
          {
            stepper.setSpeed(-1500);         // sets speed in forward direction of stepper 
            stepper.runSpeed();             // runs the step as per speed
          }
