...
void setup() {
  ...
  gpio_pull_up(5);
  if (MasterInit == 1){     //Master Setup: Team1 is configured to Master on I2C0 channel
    Wire.end();  
    Wire.begin(); 
    Serial.begin(9600);

  }   
  else if(MasterInit == 0){            //Slave Setup: Team1 is configured to Slave on I2C0 channel
    Wire.end();
    Wire.begin(TEAM1);
    Serial.begin(9600); 
...
void receiveEvent(int x){   //in wire.receiveEvent definition, an int must be de
  int Slave1Receives = Wire.read();       
  //reads handover byte from master1
  if (Slave1Receives == 3 ){          //if team1 as slave receives a 3 from other team acting as master
    Serial.println(3);
...
void loop() {
  ...
  if (MasterInit==1){        
    ...
    if (MASTERswitchSTATE == 0) {
      ...
      Wire.beginTransmission(Z_MOTION);  //Example Z motion transmission      Serial.println("Transmission initialised!");
      char header = 'a';
      int x = 3200;            
      int y = 100;    // value to encode
      ...
      //Wire.write(coord_arr,8);    //this implies coord_arr is an array of bytes, think the 8 is for no.of bits 
